...
(async () => {
  ...
  await chrome.alarms.create('syncDataCron', {
    delayInMinutes: 1,
    periodInMinutes: 10,
  });
...
async function onStartup() {
  ...
  const { pageList, lastSynced } = await chrome.storage.local.get([
    'pageList',
    'lastSynced',
  ]);
  if (pageList) {
...
async function onInstalled(details: chrome.runtime.InstalledDetails) {
  ...
  if (details.reason === 'install' || details.reason === 'update') {
    if (
      ...
      (details.previousVersion === '2.5.0' ||
        details.previousVersion === '2.7.0')
    ) {
...
async function messageEventListener(
  request: any,
...
async function sendData(visits: Page | Page[]) {
  try {
    const { user_email, pageList } = await chrome.storage.local.get([
      'user_email',
      'pageList',
    ]);
    ...
    log.debug('[syncDataCron] Response:', response);
    const data = await response.json();

    if (data.status !== 200) {
      throw new Error(
        `[sendData] Server returned ${data.status} ${data.message}`
      );
    }
...
// This method is to sync the data
// that where NOT SYNCED BEFORE
// due to some reason like API failure or network issue
// from the server to the extension
async function syncDataCron() {
  try {
    const { user_email, pageList } = await chrome.storage.local.get([
      'user_email',
      'pageList',
    ]);
    log.debug('[syncDataCron] user_email', user_email);
    log.debug('[syncDataCron] pageList', pageList);

    const userTimeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;

    const unsynced_data = (pageList as Page[]).filter(
      (page: Page) => !page.synced
    );
    log.debug('[syncDataCron] unsynced_data', unsynced_data);

    const requestBody = {
      email: user_email,
      date: new Date(),
      timeZone: userTimeZone,
      data: [unsynced_data].flat() satisfies Page[],
      version: chrome.runtime.getManifest().version,
    };
    ...
    if (data.status !== 200) {
      throw new Error(
        `[syncDataCron] Server returned ${data.status} ${data.message}`
      );
    }

    log.debug('[syncDataCron] Data sent successfully:', data);

    const updatedPageList = (pageList as Page[]).map((page: Page) => {
      if (
        unsynced_data.some(
          (unsyncedPage: Page) =>
            unsyncedPage.openedAt === page.openedAt &&
            unsyncedPage.page === page.page &&
            unsyncedPage.lastVisited === page.lastVisited &&
            unsyncedPage.synced === false
        )
      ) {
        page.synced = true;
      }
    ...
    await chrome.storage.local.set({ pageList: updatedPageList });
    await chrome.storage.local.set({ lastSynced: new Date().toISOString() });
  } catch (error) {
    log.error('[syncDataCron] Error on syncData', error);
  }
}
